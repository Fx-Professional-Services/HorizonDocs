...
- Confirming the sales order automatically increases the demand and sold quantity for each line item. To verify this, go to **Item Management > Products** and select the product to see if the inventory has increased.
- Once a sales order is confirmed, changes to the Sales Channel, Customer Tier, or Payment Terms cannot be made. To revert the confirmation, click **Unconfirm**.
