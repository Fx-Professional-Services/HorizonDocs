...
* To delete or modify a specific line item, click the delete button (x) next to that item.
	...
	⚠️ Note: The deletion will not proceed if there is an addressed line item on the demand, even if all demands were addressed or only a part of it  To unaddress a demand, see step 8.2.

   💡 Tip: Click **Unconfirm** to reverse all actions, including deleting or modifying line items. 
...
- **Assign Yielded Item to the specified location  (+)
	...
	💡 Tip: Click **Unconfirm** to reverse all actions, including
	- Deducting added items.
	- Deleting created locations.
	- Reversing updates.
